// -----------------------------------------------------------------------
// <copyright file="IEditableLookup{T1,T2,T3,T4,T5,TValue}.cs" company="Abbotware, LLC">
...
{
    /// <summary>
    ///     Interface for an editable 5-key/level look up structure
    /// </summary>
    ...
    public interface IEditableLookup<T1, T2, T3, T4, T5, TValue> : ILookup<T1, T2, T3, T4, T5, TValue>
        where T1 : notnull
        where T2 : notnull
        where T3 : notnull
        where T4 : notnull
        where T5 : notnull
    {
        ...
        void Add(T1 key1, T2 key2, T3 key3, T4 key4, T5 key5, TValue value);
    }
}