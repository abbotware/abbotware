    ...
    where TItem : class
{
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Interlocked.CompareExchange(ref this.dequeueTimeouts, 0, 0);
    }
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Interlocked.CompareExchange(ref this.enqueueTimeouts, 0, 0);
    }
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Interlocked.CompareExchange(ref this.queueHead, 0, 0);
    }
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Interlocked.CompareExchange(ref this.queueTail, 0, 0);
    }
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Interlocked.CompareExchange(ref this.queue, null, null);
    }
    ...
    public virtual bool Enqueue(ref TItem element, HighResolutionTimeSpan timer)
    {
        ...
        for (var queueCopy = this.Queue; ;)
        {
            ...
            if (tail_index >= 0)
            {
                ...
                if (used_count < this.QueueSize)
                {
                    var queue_index = tail_index % this.QueueSize;
                    ...
                    if (expected_index == tail_index)
                    {
                        ...
                        finally
                        {
                            ...
                            Debug.Assert(expected_compliment == lock_value, "expected compliment == lock value");
                        }
                    }
                }
            }
            ...
            if (timer.IsExpired)
            {
                _ = Interlocked.Increment(ref this.enqueueTimeouts);
                return false;
            }
        }
    }
    ...
    public virtual TItem? Dequeue() =>
        ...
        this.Dequeue(out var reference_type) ? reference_type : null;
    ...
    public virtual bool Dequeue(out TItem? item)
    {
        ...
        for (var try_count = 0; try_count < 100; ++try_count)
        {
            ...
            if (head_index >= 0)
            {
                ...
                var queue_index = head_index % this.QueueSize;
        ...
        _ = Interlocked.Increment(ref this.dequeueTimeouts);
        return false;
    }
    ...
    protected static bool IsLocked(int value)
        => value < 0; // all negative numbers are considered lock patterns
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!this.IsDisposed)
        {
            if (disposing)
            {
                _ = Interlocked.Exchange(ref this.queue, null);
            }
